...
pub(crate) fn qa_open(
    ...
    if let Some(path) = &qa.output {
        ...
        let player = extra.player.iter().next().map(|(pose,input)|serde_json::json!({
            "position":pose.translation.to_array(),"moving":input.active,"direction":input.direction.to_array()}));
        let actor_rows = actors
            .iter()
            .map(|(unit, transform)| {
                serde_json::json!({
            "unit":unit.0,"position":transform.translation.to_array()})
            })
            .collect::<Vec<_>>();
