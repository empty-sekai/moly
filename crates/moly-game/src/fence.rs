...
use moly_law::fixture::fence::{Neighbor, connected_parts, neighbor_positions};
...
pub(super) fn bind_scene(
    ...
    while let Some(entity) = stack.pop() {
        ...
        let Ok(extras) = extras.get(entity) else { continue; };
        ...
        let Some(active) = value.get("fenceActive").and_then(|v| v.as_bool()) else { continue; };
        ...
        if let Some(part) = value.get("fencePart") {
            let pole = match part.get("kind").and_then(|v| v.as_str()) {
                Some("pole") => true, Some("wing") => false,
                _ => panic!("FenceView part kind is invalid"),
            };
            let index = part.get("type").and_then(|v| v.as_u64()).filter(|&v| v < 9)
                .expect("FenceView part type must be 0..8") as usize;
            parts.push(Part { entity, pole, index });
        }
    }
    if fence {
...
pub(super) fn update_connections(
    ...
    if !ready.is_added() && !placements.is_changed() && !fences.iter().any(|(_, _, nodes)| nodes.is_added()) {
        return;
    }
    ...
    for (_, index, nodes) in &fences {
        ...
        let neighbors = positions.map(|p| {
            ...
            let occupants: Vec<_> = rows.iter().filter(|(_, _, _, bounds)|
                p.x >= bounds.min.x && p.x <= bounds.max.x && p.z >= bounds.min.z && p.z <= bounds.max.z
                    && p.y == bounds.min.y).collect();
            Neighbor {
                // These seven source fixture IDs map one-to-one to packages;
                // color choices live inside the same package and still connect.
                direction: occupants.first().filter(|r| r.2.package == row.package).map(|r| r.2.direction),
                joint_uids: occupants.iter().map(|r| r.1).collect(),
            }
        });
        let resolved = connected_parts(row.direction, &neighbors);
        for part in &nodes.0 {
            let active = if part.pole { resolved.poles[part.index] } else { resolved.wings[part.index] };
            commands.queue(SetSourceActive { entity: part.entity, active });
        }
        info!("围栏连接：row {} {:?} poles {:?} wings {:?}", index.0, row.direction, resolved.poles, resolved.wings);
    }
}
