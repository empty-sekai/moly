            ...
            on_ended_closure
                ...
                .replace(Closure::wrap(Box::new(move || {
                    ...
                    let time_at_start_of_buffer = {
                        ...
                        // A long task can exhaust both queued buffers. WebAudio plays
                        // expired deadlines immediately; retaining them would overlap
                        // the two buffers while the cursor catches up. Recover against
                        // the actual output clock without skipping any source samples.
                        scheduling::next_buffer_time(*time_at_start_of_buffer, now)
                    };
                    ...
                    // Decoder initialization/copying may itself exhaust the deadline.
                    // Recheck after that work, using the same initial lead as upstream.
                    let time_at_start_of_buffer = scheduling::next_buffer_time(
                        time_at_start_of_buffer,
                        ctx_handle.current_time(),
                    );
                    source
