...
export async function selectRenderer(requested, { navigator, document }, timeoutMs = 10_000) {
  ...
  if (requested !== "webgl2") {
    ...
    try {
      ...
      const adapter = await Promise.race([
        ...
        new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("WebGPU adapter request timed out")), timeoutMs); }),
      ]);
  ...
  try {
    ...
    return { backend: "webgl2", notice: requested === "auto" ? gpuFailure : "" };
  } catch (error) {
...
export async function holdSettingsWriter(locks) {
  ...
  return new Promise(resolve => {
    try {
      locks.request("moly-settings-write", { mode: "exclusive", ifAvailable: true }, async lock => {
