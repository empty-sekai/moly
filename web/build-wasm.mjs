...
function resolveTool(envName, command) {
  ...
  if (configured) {
    if (!existsSync(configured)) {
      throw new Error(`${envName} points at a missing executable: ${configured}`);
    }
    return configured;
  }
  const onPath = findOnPath(command);
  if (onPath) return onPath;
  throw new Error(`${command} not found; set ${envName} to its executable path`);
}
...
if (metadata.status !== 0) {
  throw new Error(`cargo metadata exited with status ${metadata.status}: ${metadata.stderr}`);
}
...
const lockMatch = lockText.match(/name = "wasm-bindgen"\r?\nversion = "([^"]+)"/);
if (!lockMatch) {
