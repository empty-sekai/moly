...
浏览器版使用常驻的内容目录与演出区域，可按角色、台词、家具名称搜索，浏览
对话、家具、家具故事和角色互动。完整台词按原始顺序显示，播放状态和停止按钮
始终可见；浏览其他条目不会打断演出。支持深链接、中文输入、键盘选择、明暗
主题、专注与全屏模式。按 `/` 或 `Ctrl K` 搜索，`Ctrl Enter` 播放，`Esc` 停止。

网页可以直接运行，也可通过 [嵌入接口](web/EMBEDDING.md) 接入已有站点；
嵌入模式由宿主提供页面导航。网页测试与实机验收步骤见 [浏览器检查](web/TESTING.md)。
...
视图相互一致时才进入临时布置。墙纸、地板沿用原始房间材质和 UV 选择规则
呈现，自定义表面使用原始配置；缺少源资源的条目会明确说明原因。
